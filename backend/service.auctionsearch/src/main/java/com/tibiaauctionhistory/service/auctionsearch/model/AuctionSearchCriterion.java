...
import static com.tibiaauctionhistory.service.auctionsearch.model.Operator.*;
...
public class AuctionSearchCriterion {
    ...
    public boolean usesAnyField(List<String> fields){
        final boolean usesFieldDirectly = Optional
                .ofNullable(this.field)
                .map(fields::contains)
                .orElse(false);
        final boolean hasCriteriaWithField = Optional
                .ofNullable(this.criterias)
                .map(criterias -> criterias.stream().anyMatch(criteria -> criteria.usesAnyField(fields)))
                .orElse(false);
        return usesFieldDirectly || hasCriteriaWithField;
    }
}
