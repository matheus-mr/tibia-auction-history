...
public class AbstractIntegrationTests {
    ...
    protected String KNIGHT_AUCTIONS_SEARCH_ID = NanoIdUtils.randomNanoId();

    protected final static MongoDBContainer mongoDBContainer = new MongoDBContainer("mongo:7.0.0");

    protected final static RedisContainer redisContainer = new RedisContainer("redis:7.2.5");
    ...
    public void postConstruct(){
        this.mockMvc = MockMvcBuilders
                ...
                .build();
        this.mongoClient = MongoClients.create(mongoDBContainer.getConnectionString());

        insertAuctionsToDatabase();
        insertAuctionSearchesToDatabase();
        createAuctionDomainView();
        createIndexes();
    }
    ...
    private void createAuctionDomainView(){
    ...
    private void createIndexes(){
        mongoClient
                .getDatabase(dbName)
                .getCollection(Auction.AUCTIONS_COLLECTION_NAME)
                .createIndex(
                        new Document("sold", 1),
                        new IndexOptions().name(Auction.NON_ARRAY_FIELDS_INDEX_NAME)
                );
    }
}
